...
void init_matrix(double* m, int rows, int columns) {
    #pragma omp parallel shared(m, rows, columns)
...
void mult_matrix(double* out, double* left, double *right, int rows_left, int cols_left, int cols_right) {
    int soma;
    #pragma omp parallel shared(out, left, right, rows_left, cols_left, cols_right)
                ...
                for (int j = 0; j < cols_right; ++j) {
                    out[i*cols_right+j] = 0;
                        for (int k = 0; k < cols_left; ++k) {
                            soma += left[i*cols_left+k]*right[k*cols_right+j];
                        }

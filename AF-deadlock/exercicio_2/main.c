...
void transferir(conta_t *origem, conta_t *destino, double valor) {
    pthread_mutex_t *primeiro_mutex;
    pthread_mutex_t *segundo_mutex;

    if (origem->id < destino->id) {  // Atribuindo ordem aos mutexes com base na grandeza dos IDs
        primeiro_mutex = &destino->mutex;
        segundo_mutex = &origem->mutex;
    } else {
        primeiro_mutex = &origem->mutex;
        segundo_mutex = &destino->mutex;
    }
